...
* [OpenShift Getting Started on IBM Cloud](/docs/openshift?topic=openshift-openshift_tutorial)
* [Documentation & Getting Started](/docs/openshift?topic=openshift-openshift_tutorial) on OpenShift on IBM Cloud 
...
* Support for provisioning instances into [VPC on classic](/docs/vpc-on-classic?topic=vpc-on-classic-getting-started)
...
* More secured communication between the {{site.data.keyword.cfee_full_notm}} instance and the supporting services (_Kubernetes cluster_, _Cloud Object Storage_ and _Databases for PostgreSQL_). Communication with the _Kubernetes cluster_ takes place by default through the cluster master (instead of through the ingres router). Communication with the _Cloud Object Storage_ service instance always takes place through a private access endpoint. Communication with the _Databases for PostgreSQL_ instance takes place through a private endpoint if the {{site.data.keyword.cloud_notm}} account is enabled for Virtual Routing & Fowarding ([VRF](/docs/infrastructure/direct-link?topic=direct-link-overview-of-virtual-routing-and-forwarding-vrf-on-ibm-cloud)) and IBM [Cloud Service Endpoint](/docs/service-endpoint?topic=service-endpoint-getting-started#getting-started). Otherwise, communication with  _Databases for PostgreSQL_ instance takes place through a public endpoint.
...
* New documentation is available with [managing](/docs/cloud-foundry-public?topic=cloud-foundry-public-management-enablement) and [troubleshooting](/docs/cloud-foundry-public?topic=cloud-foundry-public-debug_overview) guidance.
...
* New autoscaling capability that automatically scales Cloud Foundry application instances based on custom rules. Autoscaling is accessible from the CLI and from the Stratos console. The Stratos console can be optionally installed and launched from the _Overview_ page in the {{site.data.keyword.cfee_full_notm}} user interface. In the _Applications_ page of the Stratos console, select an  application and look for the **Auto Scaling** tab. Click *Create Policy* to launch the autoscaling editor to define the scaling policy for the target application.
  See the [autoscaling documentation](/docs/cloud-foundry-public?topic=cloud-foundry-public-autoscale_cloud_foundry_apps#autoscale_cloud_foundry_apps) for more information.
...
* New capabilities:
   * {{site.data.keyword.cfee_full_notm}} instances can be provisioned on [virtual-dedicated](/docs/containers/cs_clusters.html#clusters#clusters_ui_standard) Kubernetes worker nodes hosted on infrastructure that is devoted to your {{site.data.keyword.cloud_notm}} account.
   * A new data center in Tokyo (tok05) is available for provisionng {{site.data.keyword.cfee_full_notm}} instances.
   * [Updating](/docs/cloud-foundry-public/updating-scaling.html#update-scale#update) an {{site.data.keyword.cfee_full_notm}} instance to a new version.
   * [Scaling](/docs/cloud-foundry-public/updating-scaling.html#update-scale#scale) the capacity of an {{site.data.keyword.cfee_full_notm}} instance by adding or deleting Cloud Foundry cells.
   * [Auditing](/docs/cloud-foundry-public/auditing-logging.html#auditing-logging#auditing) Cloud Foundry activities through integration with an automatically configured IBM Cloud Activity Tracker service instance.
   * Persisting Cloud Foundry [log events](/docs/cloud-foundry-public/auditing-logging.html#auditing-logging#logging) through an automatically configured IBM Cloud Log Analysis service instance.
   ...
   * New commands to perform {{site.data.keyword.cfee_full_notm}} related actions on the command line interface (CLI):
     ...
     * `ibmcloud cfee create-permission-get` and `ibmcloud cfee create-permission-set`  [commands](/docs/cloud-foundry-public/permissions.html#permissions#permcli-creating) to retrieve and set the permissions required to create instances. The commands aggregate and simplify the setting of permissions for the {{site.data.keyword.cfee_full_notm}} service and for the required supporting services.
     * `ibmcloud catalog blacklist` command to simplify [visibility control](/docs/cloud-foundry-public/add-serv-inst.html#workingwith-services#service_visibility) of {{site.data.keyword.cloud_notm}} services for users in an {{site.data.keyword.cloud_notm}} account.
